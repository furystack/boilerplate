...
test.describe('Example Application', () => {
  test('Login and logout roundtrip', async ({ page }) => {
    ...
    const loginForm = page.locator('shade-login>div>form')
    ...
    const submitButton = page.locator('shade-login button')
    ...
    const logoutButton = page.locator('shade-app-bar shade-button button >> text="Log Out"')
    await expect(logoutButton).toBeVisible()
    await expect(logoutButton).toBeEnabled()
    await expect(logoutButton).toHaveText('Log Out')
    await logoutButton.click()

    const loggedOutLoginForm = page.locator('shade-login>div>form')
