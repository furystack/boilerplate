...
### Migrated `NestedRouteLink` from `href` to `path`

`@furystack/shades` v14 renamed the `NestedRouteLink` `href` prop to `path` to align with the rest of the routing APIs (`Breadcrumb`, `nestedNavigate`, etc.). Updated the header navigation link in `components/header.tsx` accordingly.
