...
- Bumped `@furystack/rest` from `^8.1.3` to `^9.0.0`. The removed `SwaggerDocument` type and `/swagger.json` endpoint on `WithSchemaAction` are not used in this package, so no code changes were needed.
